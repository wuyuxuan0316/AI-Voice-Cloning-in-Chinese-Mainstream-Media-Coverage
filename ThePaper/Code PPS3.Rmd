...
# People's Daily S1
```{r}
PPS3 <- read_lines("ThePaperS3.txt", locale = locale(encoding = "UTF-8"))

```

```{r}

corpus <- Corpus(VectorSource(PPS3))
...
writeLines(texts, "text/processed_ThePaperS3.txt")
...
dtm <- DocumentTermMatrix(seg_corpus, control = list(wordLengths = c(1, Inf)))
m <- as.matrix(dtm)
freqs <- sort(colSums(m), decreasing = TRUE)
df1 <- data.frame(word = names(freqs), freq = freqs)
...
```{r, include=FALSE}
# 保存为 CSV 文件
write.csv(df3, "text/word_PPS3.csv", row.names = FALSE)
```
...
```{r}
#读取经过翻译处理的词频文件
df4 <- read_csv("text/translated_PPS3.csv")
#df5 <- df4 %>%
#  filter(freq >= 60) 
df4 <- df4 %>%
...
p1 <- ggplot(df5, aes(x = reorder(word, freq), y = freq)) +
  ...
  theme(axis.text.y = element_text(size = 8),  # 减小y轴字体大小
        axis.title.y = element_text(angle = 0, vjust = 0.5))  # 调整y轴标题的位置和角度
...
print(p1)
print(p1e)
ggsave("image/freqPPS3.png", plot = p1, width = 10, height = 8, dpi = 300)
ggsave("image/freqPPS3e.png", plot = p1e, width = 10, height = 8, dpi = 300)
```
...
p2 <- ggplot(df_filtered, aes(x = emotion, y = total_freq, fill = emotion)) +
  ...
  theme_minimal() +
  labs(x = "Emotion", y = "Frequency", title = "Stage3-The Paper-Sentiment") +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, max(df_filtered$total_freq), by = 500))

ggsave("image/emotionPPS3.png", plot = p2, width = 10, height = 8, dpi = 300)
print(p2)
```
...
print(p3e)
ggsave("image/sentimentPPS3e.png", plot = p3e, width = 10, height = 8, dpi = 300)

print(p3)
ggsave("image/sentimentPPS3.png", plot = p3, width = 10, height = 8, dpi = 300)
...
ggsave("image/negative_sentiment_PDS3.png", plot = p_negative, width = 10, height = 8, dpi = 300)
ggsave("image/negative_sentiment_PDS3e.png", plot = pe_negative, width = 10, height = 8, dpi = 300)
```
