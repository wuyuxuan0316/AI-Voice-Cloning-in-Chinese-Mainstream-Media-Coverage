...
# People's Daily S1
```{r}
PPS2 <- read_lines("ThePaperS2.txt", locale = locale(encoding = "UTF-8"))

```

```{r}

corpus <- Corpus(VectorSource(PPS2))
...
writeLines(texts, "text/processed_ThePaperS2.txt")
...
```{r, include=FALSE}
# 保存为 CSV 文件
write.csv(df3, "text/word_PPS2.csv", row.names = FALSE)
```
...
```{r}
#读取经过翻译处理的词频文件
df4 <- read_csv("text/translated_PPS2.csv")
#df5 <- df4 %>%
#  filter(freq >= 60) 
df4 <- df4 %>%
...
print(p1)
print(p1e)
ggsave("image/freqPPS2.png", plot = p1, width = 10, height = 8, dpi = 300)
ggsave("image/freqPPS2e.png", plot = p1e, width = 10, height = 8, dpi = 300)
```
...
p2 <- ggplot(df_filtered, aes(x = emotion, y = total_freq, fill = emotion)) +
  ...
  theme_minimal() +
  labs(x = "Emotion", y = "Frequency", title = "Stage2-The Paper-Sentiment") +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, max(df_filtered$total_freq), by = 500))

ggsave("image/emotionPPS2.png", plot = p2, width = 10, height = 8, dpi = 300)
print(p2)
```
...
print(p3e)
ggsave("image/sentimentPPS2e.png", plot = p3e, width = 10, height = 8, dpi = 300)

print(p3)
ggsave("image/sentimentPPS2.png", plot = p3, width = 10, height = 8, dpi = 300)
...
```{r}
# Filter data for negative sentiment and frequency >= 2
df_negative <- df4 %>%
...
ggsave("image/negative_sentiment_PDS2e.png", plot = pe_negative, width = 10, height = 8, dpi = 300)
```
