...
# People's Daily S1
```{r}
PDS3 <- read_lines("People'sDailyS3.txt", locale = locale(encoding = "UTF-8"))

```

```{r}

corpus <- Corpus(VectorSource(PDS3))
...
writeLines(texts, "text/processed_People'sDailyS3.txt")
...
```{r, include=FALSE}
# 保存为 CSV 文件
write.csv(df3, "text/word_PDS3.csv", row.names = FALSE)
```
...
```{r}
#读取经过翻译处理的词频文件
df4 <- read_csv("text/translated_PDS3.csv")
df4 <- df4 %>%
...
print(p1)
print(p1e)
ggsave("image/freqPDS3.png", plot = p1, width = 10, height = 8, dpi = 300)
ggsave("image/freqPDS3e.png", plot = p1e, width = 10, height = 8, dpi = 300)
```
...
p2 <- ggplot(df_filtered, aes(x = emotion, y = total_freq, fill = emotion)) +
  ...
  theme_minimal() +
  labs(x = "Emotion", y = "Frequency", title = "Stage3-People's Daily-Sentiment") +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, max(df_filtered$total_freq), by = 50))

ggsave("image/emotionPDS3.png", plot = p2, width = 10, height = 8, dpi = 300)
print(p2)
```
...
ggsave("image/sentimentPDS3.png", plot = p3, width = 10, height = 8, dpi = 300)
ggsave("image/sentimentPDS3e.png", plot = p3e, width = 10, height = 8, dpi = 300)
```
