...
# People's Daily S1
```{r}
PDS1 <- read_lines("People'sDailyS1.txt", locale = locale(encoding = "UTF-8"))

```

```{r}

corpus <- Corpus(VectorSource(PDS1))
...
writeLines(texts, "text/processed_People'sDailyS1.txt")
...
```{r, include=FALSE}
# 保存为 CSV 文件
write.csv(df3, "text/word_PDS1.csv", row.names = FALSE)
```
...
```{r}
#读取经过翻译处理的词频文件
df4 <- read_csv("text/translated_PDS1.csv")
df4 <- df4 %>%
...
print(p1)
print(p1e)
ggsave("image/freqPDS1.png", plot = p1, width = 10, height = 8, dpi = 300)
ggsave("image/freqPDS1e.png", plot = p1e, width = 10, height = 8, dpi = 300)
```
...
p2 <- ggplot(df_filtered, aes(x = emotion, y = total_freq, fill = emotion)) +
  ...
  theme_minimal() +
  labs(x = "Emotion", y = "Frequency", title = "Stage1-People's Daily-Sentiment") +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = seq(0, max(df_filtered$total_freq), by = 50))

ggsave("image/emotionPDS1.png", plot = p2, width = 10, height = 8, dpi = 300)
print(p2)
```
...
ggsave("image/sentimentPDS1.png", plot = p3, width = 10, height = 8, dpi = 300)
ggsave("image/sentimentPDS1e.png", plot = p3e, width = 10, height = 8, dpi = 300)
```
...
p_negative <- ggplot(df_negative, aes(x = reorder(word, freq), y = freq, fill = sentiment)) +
  ...
  geom_text(aes(label = freq, hjust = -0.3), color = "black", size = 3) +
